...
where X is the type of database to convert.  Currently recognized:
...
Set the trace level on startup, equivilant of using the OperServ
...
(for services failover purposes, higher level = less likely to
be failed to).
...
I<NOTE: if your F<magick.key> file was created with a different
f<magick_keygen> binary than the one associated with the current
version of F<magick> you are running (including the actual COMPILE
of it), you will essentially create corrupted databases>
...
your F<magick.key> file to have the correct key do decrypt the
databases (and to have been created with the F<magick_keygen> of
the same compile set as the current F<magick> binary).

=item --keyfile X, -K

Override the B<FILES/KEYFILE> value in the configuration file,
which is the file that contains the key to decrypt or encrypt
...
F<magick> that were compiled at the same time.  This defaults to
F<magick.key>.
...
channel modes, who we need to rename because they havnt ident'd,
...
which tells magick to log all messages recieved by a user that is
