...
int mMessage::call()
{
	    ...
	    !IsChan(IrcParam(params_, 1)))
	{
		...
		if (serv != NULL)
		{
		    mstring msg(IrcParam(params_, 2));
		    MLOCK((lck_OperServ, "KillPhrase"));
		    if (Magick::instance().operserv.KillPhrase_find(msg) &&
			!((Magick::instance().commserv.IsList(Magick::instance().commserv.OPER_Name()) &&
			 Magick::instance().commserv.GetList(Magick::instance().commserv.OPER_Name())->IsOn(src)) ||
