...
void load_cs_dbase(void)
{
	...
	for (i = 33; i < 256; ++i)
		...
		if (ci->accesscount)
		{
		    ...
		    ci->access = i_access;
		    if (ci->accesscount != (long) ACE_OS::fread(i_access, sizeof(ChanAccess), ci->accesscount, f))
		    {
			SLOG(LM_EMERGENCY, "Read error on $1", (chanserv_db));
		    }
		    ...
		    if (ci->accesscount)
			ci->access = (ChanAccess *) realloc(ci->access, sizeof(ChanAccess) * ci->accesscount);
		    else
		    {
			free(ci->access);
			ci->access = NULL;
		...
		if (ci->akickcount)
		{
		    ...
		    ci->akick = akick;
		    if (ci->akickcount != (long) ACE_OS::fread(akick, sizeof(AutoKick), ci->akickcount, f))
		    {
			SLOG(LM_EMERGENCY, "Read error on $1", (chanserv_db));
		    }
		    ...
		    if (ci->akickcount)
		    {
			ci->akick = (AutoKick *) realloc(ci->akick, sizeof(AutoKick) * ci->akickcount);
		    }
		    else
		    {
			free(ci->akick);
		...
		if (ci->cmd_access)
		{
			...
			ACE_OS::fseek(f, sizeof(short) * (n_entries - CA_SIZE), SEEK_CUR);
		    }
		}
...
Chan_Stored_t *CreateChanEntry(ChanInfo_CUR * ci)
{
	...
	if (!out->L_Revenge())
	{
	    char revlevel = ((ci->flags & CI_REV3) << 2) | ((ci->flags & CI_REV2) << 1) | (ci->flags & CI_REV1);
...
void load_message()
{
	...
	for (j = 0; j < nmessage; ++j)
	{
	    ...
	    if (messages[j].type == M_LOGON && Magick::instance().commserv.IsList(Magick::instance().commserv.ALL_Name()))
	    {
	    ...
	    else if (messages[j].type == M_OPER && Magick::instance().commserv.IsList(Magick::instance().commserv.OPER_Name()))
	    {
