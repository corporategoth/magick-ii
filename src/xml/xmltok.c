...
static const struct normal_encoding utf8_encoding_ns =
{
    ...
    ,
    {
#include "xml/asciitab.h"
#include "xml/utf8tab.h"
    }
...
static int doParseXmlDecl(const ENCODING * (*encodingFinder) (const ENCODING *, const char *, const char *),
			  int isGeneralTextEntity, const ENCODING * enc, const char *ptr, const char *end, const char **badPtr,
...
int XmlUtf8Encode(int c, char *buf)
{
    ...
    if (c < min3)
    {
	buf[0] = ((c >> 8) | UTF8_cval2);
	buf[1] = ((c & 0x3f) | 0x80);
	return 2;
    }
    if (c < min4)
    {
	buf[0] = ((c >> 16) | UTF8_cval3);
	buf[1] = (((c >> 8) & 0x3f) | 0x80);
	buf[2] = ((c & 0x3f) | 0x80);
	return 3;
    }
    if (c < 0x110000)
    {
	buf[0] = ((c >> 24) | UTF8_cval4);
	buf[1] = (((c >> 16) & 0x3f) | 0x80);
	buf[2] = (((c >> 8) & 0x3f) | 0x80);
	buf[3] = ((c & 0x3f) | 0x80);
	return 4;
    }
...
static void unknown_toUtf16(const ENCODING * enc, const char **fromP, const char *fromLim, unsigned short **toP,
			    const unsigned short *toLim)
{
    while (*fromP != fromLim && *toP != toLim)
    {
	...
	if (c == 0)
	{
	    c = (unsigned short) ((const struct unknown_encoding *) enc)->convert(((const struct unknown_encoding *) enc)->
										  userData, *fromP);
	    *fromP += ((const struct normal_encoding *) enc)->type[(unsigned char) **fromP] - (BT_LEAD2 - 2);
	}
	else
