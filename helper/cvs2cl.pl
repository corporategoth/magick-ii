...
## MODIFIED BY Preston A. Elder <prez@magick.tm>
##
## This script has been modified to change the output formatting to how
## I like it.  If you want it, take it, its GPL'd :)
...
sub maybe_grab_accumulation_date ()
{
  ...
  while (<LOG>)
  {
    if (/^\*\s+(\d\d\d\d-\d\d-\d\d\s+\d\d:\d\d)/)
    {
      $boundary_date = "$1";
      last;
    }
